...
- (void)viewDidLoad {
    ...
    NSArray* inputTypeData = [[NSArray alloc] initWithObjects:
                              @"Telex",
                              @"VNI",
                              @"Simple Telex",
                              @"Telex Tây Nguyên", nil];
...
- (void)viewDidAppear {
    [super viewDidAppear];
    NSString* str = @"TnKey %@ - Bộ gõ Tây Nguyên";
...
- (IBAction)onHomePageLink:(id)sender {
    [[NSWorkspace sharedWorkspace] openURL: [NSURL URLWithString:@"https://www.tn-key.com/"]];
}
...
- (IBAction)onEmailLink:(id)sender {
    [[NSWorkspace sharedWorkspace] openURL: [NSURL URLWithString:@"mailto:thamdavies@gmail.com"]];
}
