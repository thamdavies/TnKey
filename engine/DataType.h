...
#define IS_CONSONANT_TN(keyCode) !(keyCode == KEY_A || keyCode == KEY_E || keyCode == KEY_U || keyCode == KEY_I || keyCode == KEY_O)
...
#define IS_SPECIALKEY(keyCode) \
        ...
        : (vInputType == vTelex_TN ? \
            keyCode == KEY_A || keyCode == KEY_O || keyCode == KEY_E || keyCode == KEY_X || keyCode == KEY_F || \
            keyCode == KEY_D || keyCode == KEY_C || keyCode == KEY_N || keyCode == KEY_B || keyCode == KEY_Z || keyCode == KEY_Q : false))))

//is VNI or Unicode compound...
#define IS_DOUBLE_CODE(code) (code == 2 || code == 3|| code == 5)
#define IS_VNI_CODE(code) (code == 2)
#define IS_QUICK_TELEX_KEY(code) (_index > 0 && ( code == KEY_C || code == KEY_N || code == KEY_G || code == KEY_K || code == KEY_P || code == KEY_Q || code == KEY_T || code == KEY_U) && \
