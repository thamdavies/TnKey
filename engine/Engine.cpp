...
static Uint16 ProcessingChar[][11] = {
    ...
    {KEY_X, KEY_F, KEY_C, KEY_N, KEY_B, KEY_A, KEY_O, KEY_E, KEY_Q, KEY_D, KEY_Z} //Telex_TN
};

#define IS_KEY_Z(key) (ProcessingChar[vInputType][10] == key)
#define IS_KEY_D(key) (ProcessingChar[vInputType][9] == key)
#define IS_KEY_C(key) (KEY_C == key)
#define IS_KEY_N(key) (KEY_N == key)
#define IS_KEY_B(key) (KEY_B == key)
#define IS_KEY_W(key) ((vInputType == vTelex || vInputType == vSimpleTelex || vInputType == vTelex_TN) ? ProcessingChar[vInputType][8] == key : \
                                    (vInputType == vVNI ? (ProcessingChar[vInputType][8] == key || ProcessingChar[vInputType][7] == key) : false))
#define IS_KEY_DOUBLE(key) ((vInputType == vTelex || vInputType == vSimpleTelex || vInputType == vTelex_TN) ? (ProcessingChar[vInputType][5] == key || ProcessingChar[vInputType][6] == key || ProcessingChar[vInputType][7] == key) :\
...
#define IS_KEY_R(key) ((vInputType == vTelex || vInputType == vVNI || vInputType == vSimpleTelex) ? ProcessingChar[vInputType][2] == key : false)
#define IS_KEY_X(key) ((vInputType == vTelex || vInputType == vVNI || vInputType == vSimpleTelex) ? ProcessingChar[vInputType][3] == key : false)
#define IS_KEY_J(key) ((vInputType == vTelex || vInputType == vVNI || vInputType == vSimpleTelex) ? ProcessingChar[vInputType][4] == key : false)
...
#define IS_MARK_KEY(keyCode) (((vInputType == vTelex || vInputType == vSimpleTelex) && (keyCode == KEY_S || keyCode == KEY_F || keyCode == KEY_R || keyCode == KEY_J || keyCode == KEY_X)) || \
                                        (vInputType == vTelex_TN && (keyCode == KEY_X || keyCode == KEY_F)) || \
...
void findAndCalculateVowelForTN(const bool& forGrammar = false);
void insertMark(const Uint32& markMask, const bool& canModifyFlag=true);
void insertMarkForTN(const Uint32& markMask, const bool& canModifyFlag = true);
...
void findAndCalculateVowelForTN(const bool& forGrammar) {
    vowelCount = 0;
    VSI = VEI = 0;
    for (iii = _index - 1; iii >= 0; iii--) {
        if (IS_CONSONANT_TN(CHR(iii))) {
            if (vowelCount > 0)
                break;
        }
        else {  //is vowel
...
void handleModernMarkForTN() {
    //default
    VWSM = VEI;
    hBPC = (_index - VEI);

    //rule 2
    if (vowelCount == 3 && (CHR(VSI) == KEY_U && CHR(VSI + 1) == KEY_A && CHR(VSI + 2) == KEY_I)) {
        VWSM = VEI;
        hBPC = (_index - VEI);
    }
    else if (vowelCount == 2 && ((CHR(VSI) == KEY_E && CHR(VSI + 1) == KEY_O) ||
        (CHR(VSI) == KEY_E && CHR(VSI + 1) == KEY_U) ||
        (CHR(VSI) == KEY_A && (TypingWord[VSI] & TONE_MASK) && CHR(VSI + 1) == KEY_O) ||
        (CHR(VSI) == KEY_O &&(TypingWord[VSI] & TONE_MASK) && CHR(VSI+1) == KEY_I ))) {
        VWSM = VSI;
        hBPC = _index - VWSM;
    }
    
    else if (vowelCount == 2 && ((CHR(VSI) == KEY_A && (CHR(VSI + 1) == KEY_I)) ||
        (CHR(VSI) == KEY_A && !(TypingWord[VSI] & TONE_MASK) && !(TypingWord[VSI] & TONEW_MASK) && (CHR(VSI + 1) == KEY_O) && !(TypingWord[VSI+1] & TONE_MASK) && !(TypingWord[VSI+1] & TONEW_MASK)) ||
        (CHR(VSI) == KEY_I && (CHR(VSI + 1) == KEY_O )) ||
        (CHR(VSI) == KEY_I && (CHR(VSI + 1) == KEY_U )) ||
        (CHR(VSI) == KEY_I && CHR(VSI + 1) == KEY_E && !(TypingWord[VSI+1] & TONE_MASK)) ||
        (CHR(VSI) == KEY_O && (TypingWord[VSI] & TONEW_MASK) && CHR(VSI+1) == KEY_I) ||
        (CHR(VSI) == KEY_U && CHR(VSI + 1) == KEY_O && (TypingWord[VSI+1] & TONEW_MASK)) ||
        (CHR(VSI) == KEY_U && CHR(VSI+1) == KEY_I) ||
        (CHR(VSI) == KEY_U && (CHR(VSI + 1) == KEY_E)))) {
        VWSM = VSI+1;
        hBPC = _index - VWSM;
    }
    else {
...
void handleModernMarkForTNForMark1() {
    //default
    VWSM = VEI;
    hBPC = (_index - VEI);

    //rule 2
    if ((CHR(VSI) == KEY_A && (TypingWord[VSI] & CAPS_MASK) && !(TypingWord[VSI] & TONE_MASK) && !(TypingWord[VSI] & TONEW_MASK)) ||
        ((CHR(VSI) == KEY_O && (TypingWord[VSI] & CAPS_MASK)) && !(TypingWord[VSI] & TONE_MASK) && !(TypingWord[VSI] & TONEW_MASK)) ||
        ((CHR(VSI) == KEY_E && (TypingWord[VSI] & CAPS_MASK)) && !(TypingWord[VSI] & TONE_MASK) && !(TypingWord[VSI] & TONEW_MASK))) {
        VWSM = VSI;
        hBPC = _index - VWSM;
    }
    else
...
void insertMarkForTN(const Uint32& markMask, const bool& canModifyFlag) {
    vowelCount = 0;
    ...
    /*if (vowelCount>2)
        hCode = vDoNothing;
    else */ if (vowelCount == 1 && markMask==MARK1_MASK) {
        handleModernMarkForTNForMark1();
    }
...
void insertC(const Uint16& data, const bool& isCaps) {
    hCode = vWillProcess;
    hBPC = 0;
    for (ii = _index - 1; ii >= 0; ii--) {
        hBPC++;
        if (CHR(ii) == KEY_C) { //reverse unicode char
            if (TypingWord[ii] & TONE_MASK) {
                //restore and disable temporary
                hCode = vRestore;
                TypingWord[ii] &= ~TONE_MASK;
...
void insertN(const Uint16& data, const bool& isCaps) {
    hCode = vWillProcess;
    hBPC = 0;
    for (ii = _index - 1; ii >= 0; ii--) {
        hBPC++;
        if (CHR(ii) == KEY_N) { //reverse unicode char
            if (TypingWord[ii] & TONE_MASK) {
                //restore and disable temporary
                hCode = vRestore;
                TypingWord[ii] &= ~TONE_MASK;
    ...
    for (ii = _index - 1; ii >= 0; ii--) {
        ...
        if (CHR(ii) == data) { //reverse unicode char
            if (TypingWord[ii] & TONE_MASK) {
                ...
                TypingWord[ii] |= TONE_MASK;
                if (!IS_KEY_D(data) && !IS_KEY_C(data) && !IS_KEY_N(data) && !IS_KEY_B(data))
                    TypingWord[ii] &= ~TONEW_MASK;
                hData[_index - 1 - ii] = GET(TypingWord[ii]);
...
void insertAOEForTN(const Uint16& data, const bool& isCaps) {
    findAndCalculateVowel();
        ...
        if (((TypingWord[VSI] & TONEW_MASK) && (TypingWord[VSI+1] & TONEW_MASK)) ||
            ...
            if ((CHR(VSI) == KEY_U && CHR(VSI+1) == KEY_O)) {
                if (VSI - 2 >= 0 && TypingWord[VSI - 2] == KEY_T && TypingWord[VSI - 1] == KEY_H) {
                    TypingWord[VSI+1] |= TONEW_MASK; //THUƠ
                    if (VSI + 2 < _index && CHR(VSI+2) == KEY_N) {
                        TypingWord[VSI] |= TONEW_MASK;  //THƯƠN
                    }
                } else if (VSI - 1 >= 0 && TypingWord[VSI - 1] == KEY_Q) {
                    TypingWord[VSI+1] |= TONEW_MASK;    //QUƠ
                } else {
                    TypingWord[VSI] |= TONEW_MASK;
                    TypingWord[VSI+1] |= TONEW_MASK;    //ƯƠ
                }
            } else if ((CHR(VSI) == KEY_U && CHR(VSI+1) == KEY_A) ||  //ƯA
                       (CHR(VSI) == KEY_U && CHR(VSI+1) == KEY_I) ||  //ƯI
                       (CHR(VSI) == KEY_U && CHR(VSI+1) == KEY_U) ||  //ƯU
                       (CHR(VSI) == KEY_O && CHR(VSI+1) == KEY_I)) {  //ƠI
                TypingWord[VSI] |= TONEW_MASK;
            } else if ((CHR(VSI) == KEY_I && CHR(VSI+1) == KEY_O) ||  //IƠ
                       (CHR(VSI) == KEY_O && CHR(VSI+1) == KEY_A)) {  //OĂ
...
void insertWForTN(const Uint16& data, const bool& isCaps) {
    isRestoredW = false;
    ...
    if (vowelCount > 1) {
        ...
        if ((TypingWord[VSI] & TONEW_MASK) ||
            ...
            if ((CHR(VSI) == KEY_A && CHR(VSI + 1) == KEY_I) ||  //ĂI
                (CHR(VSI) == KEY_A && CHR(VSI + 1) == KEY_O) ||  //ĂO
                (CHR(VSI) == KEY_O && CHR(VSI + 1) == KEY_I) ||  //ƠI
                (CHR(VSI) == KEY_U && CHR(VSI + 1) == KEY_I))    //ƯI
            {
                TypingWord[VSI] |= TONEW_MASK;
            }
            else if ((CHR(VSI) == KEY_I && CHR(VSI + 1) == KEY_A) ||  //IĂ
                (CHR(VSI) == KEY_I && CHR(VSI + 1) == KEY_O) ||  //IƠ
                (CHR(VSI) == KEY_I && CHR(VSI + 1) == KEY_U) ||  //IƯ
                (CHR(VSI) == KEY_E && CHR(VSI + 1) == KEY_U) ||  //EƯ
                (CHR(VSI) == KEY_U && CHR(VSI + 1) == KEY_O) ||  //UƠ
                (CHR(VSI) == KEY_U && CHR(VSI + 1) == KEY_A))     //UĂ
            {
                TypingWord[VSI+1] |= TONEW_MASK;
            }
            else {
...
void handleMainKey(const Uint16& data, const bool& isCaps) {
    ...
    if (IS_KEY_C(data)) {
        ...
        isChanged = false;
        k = _index;
        for (i = 0; i < _consonantC.size(); i++) {
            if (_index < _consonantC[i].size())
                continue;
            isCorect = true;
            checkCorrectVowel(_consonantC, i, k, data);

            //allow d after consonant
            if (!isCorect && _index - 2 >= 0 && CHR(_index - 1) == KEY_C && IS_CONSONANT(CHR(_index - 2))) {
                isCorect = true;
            }
            if (isCorect) {
                isChanged = true;
    ...
    if (IS_KEY_N(data)) {
        ...
        isChanged = false;
        k = _index;
        for (i = 0; i < _consonantN.size(); i++) {
            if (_index < _consonantN[i].size())
                continue;
            isCorect = true;
            checkCorrectVowel(_consonantN, i, k, data);

            //allow d after consonant
            if (!isCorect && _index - 2 >= 0 && CHR(_index - 1) == KEY_N && IS_CONSONANT(CHR(_index - 2))) {
                isCorect = true;
            }
            if (isCorect) {
                isChanged = true;
    ...
    if (IS_MARK_KEY(data)) {
        if (vInputType == vTelex_TN) {
            for (i = 0; i < _vowelForMarkForTN.size(); i++) {
                vector<vector<Uint16>>& charset = _vowelForMarkForTN[i];
                isCorect = false;
                isChanged = false;
                k = _index;
                for (l = 0; l < charset.size(); l++) {
                    ...
                    if (isCorect) {
                        isChanged = true;
                        if (IS_KEY_S(data))
                            insertMarkForTN(MARK1_MASK);
                        else if (IS_KEY_F(data))
                            insertMarkForTN(MARK2_MASK);
                        break;
                    }
                }
    ...
    keyForAEO = ((vInputType == vTelex || vInputType == vSimpleTelex) ? data : (data == KEY_Q ? KEY_W : (((data == KEY_7 || data == KEY_8 ? KEY_W : (data == KEY_6 ? TypingWord[VEI] : data))))));
    
    if (vInputType == vTelex_TN) {
        vector<vector<Uint16>>& charset = _vowelForTN[keyForAEO];
        isCorect = false;
        isChanged = false;
        k = _index;
        for (i = 0; i < charset.size(); i++) {
            ...
            if (isCorect) {
                ...
                if (IS_KEY_DOUBLE(data)) {
                    insertAOEForTN(keyForAEO, isCaps);
                }
                else if (IS_KEY_W(data)) {
                    insertWForTN(keyForAEO, isCaps);
                }
                break;
            }
        }
    ...
    if (!isChanged) {
        if (data == KEY_W && vInputType != vSimpleTelex && vInputType != vTelex_TN) {
            checkForStandaloneChar(data, isCaps, KEY_U);
        } else {
            insertKey(data, isCaps);
        }
...
void vKeyHandleEvent(const vKeyEvent& event,
    ...
    if ((IS_NUMBER_KEY(data) && capsStatus == 1)
        ...
        if (!IS_SPECIALKEY(data) || tempDisableKey) { //do nothing
            if (vQuickTelex && IS_QUICK_TELEX_KEY(data) && (vInputType != vTelex_TN)) {
